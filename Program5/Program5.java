...
{
    private static final String FILE_LOCATION = "src\\Program5\\Program5.";

    public static void main(String[] args) //throws IOException
    {
        try {
            BufferedReader fileIn = new BufferedReader(new FileReader(new File(FILE_LOCATION + "txt").getCanonicalFile()));
            BufferedWriter fileOut = new BufferedWriter(new FileWriter(new File(FILE_LOCATION + "out").getCanonicalFile()));
            String lineIn;
            ...
            while ((lineIn = fileIn.readLine()) != null) {
                ...
                fileOut.write(String.format("%-25s -> %s\n", lineIn, postFix));
        ...
        {
            System.out.println("#EXCEPITON# File IO issue.");
            e.printStackTrace();
        } catch (Exception e)
        {
            System.out.println("#EXCEPITON# NON-File issue.");
            e.printStackTrace();
        }
    }
    ...
    public static String InfixToPostFix(String input)
    {
        ...
        parseInputChars: for (int i = 0; i < input.length(); i++)
        {
            ...
            else if (checkOperator(c)) //High Precedence operators, only time it may be necessary to pop values off operatorStack.
            {
                ...
                if (operatorStack.empty()) //Added this block to account for behavior of Java Stack<> peek().
                {
